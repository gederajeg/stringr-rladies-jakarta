...
```{r melihat-hasil-pencarian, message = FALSE}
...
# menangkap karakter `"a"` yang muncul di akhir (`"$"`) rangkaian karakter nama buah pada vektor `x`
...
str_view_all(string = xx, pattern = "apple")
str_subset(string = xx, pattern = "apple")
```
...
Isunya adalah tanda baca seperti titik (`.`) masih menempel pada kata *ends.* dan *enough.*. Pada situasi inilah kekuatan dari pola pencarian dengan RegEx menjadi penting.
